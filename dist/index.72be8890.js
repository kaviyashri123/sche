...
function createTable() {
    let table = $("#table");
    let head = "";
    for(h in headers)head += "<th>" + headers[h] + "</th>";
    table.append(`<thead><tr>${head}</tr></thead>`);
    data = "";
    for(p in processes){
        let row = "";
        for(obj in processes[p])row += "<td>" + processes[p][obj] + "</td>";
        data += "<tr>" + row + "</tr>";
    }
...
function addProcess() {
    ...
    if ($("#modal_burst_time").val() < 1) $("#error_bt").removeAttr("hidden");
    else $("#error_bt").attr("hidden", true);
    if ($("#modal_arrival_time").val() < 0) $("#error_at").removeAttr("hidden");
    else $("#error_at").attr("hidden", true);
    if ($("#modal_priority").val() < 0) $("#error_pt").removeAttr("hidden");
    else $("#error_pt").attr("hidden", true);
...
function openEditModal() {
    ...
    $dropdown_edit.empty();
    for(i = 0; i < processes.length; i++)$dropdown_edit.append($("<option />").val(processes[i].id).text("Process " + processes[i].id));
...
function editProcess() {
    if (processes.length != 0) {
        if ($("#modal_edit_burst_time").val() < 1) $("#error_edit_bt").removeAttr("hidden");
        else $("#error_edit_bt").attr("hidden", true);
        if ($("#modal_edit_arrival_time").val() < 0) $("#error_edit_at").removeAttr("hidden");
        else $("#error_edit_at").attr("hidden", true);
        if ($("#modal_edit_priority").val() < 0) $("#error_edit_pt").removeAttr("hidden");
        else $("#error_edit_pt").attr("hidden", true);
        if ($("#modal_edit_burst_time").val() > 0 && $("#modal_edit_arrival_time").val() >= 0 && $("#modal_edit_priority").val() >= 0) {
            ...
            if (new_burst_time === "") new_burst_time = processes[process_id - 1].burst_time;
            if (new_arrival_time === "") new_arrival_time = processes[process_id - 1].arrival_time;
            if (new_priority === "") new_priority = processes[process_id - 1].priority;
...
function openRemoveModal() {
    ...
    for(i = 0; i < processes.length; i++)$dropdown_remove.append($("<option />").val(processes[i].id).text("Process " + processes[i].id));
// $('#modal_remove_select').html(options);
}
function removeProcess() {
    let removing_array = $("#modal_remove_select").val();
    let i1 = 0;
    for(i1 = 0; i1 < removing_array.length; i1++){
        let j = 0;
        processes.forEach(function(p1) {
            if (removing_array[i1] == p1.id) processes.splice(j, 1);
            j++;
        });
    }
    orderProcess();
...
$("#roundrobin_switch").on("change", ()=>{
    if ($("#roundrobin_switch").prop("checked") === false) $("#show_time_quanta").hide();
    if ($("#roundrobin_switch").prop("checked") === true) $("#show_time_quanta").show();
});
$("#vis_stop").on("click", ()=>{
    stop_flag = true;
});
